...
public class ServerSideView extends javax.swing.JFrame {    
    ...
    private final String[] QUESTIONSLIST = {
        "when did out bank started 1999 (yes/no) = ?",
        "the leader of our bank is called peter henderson(yes/no)= ?",
        "the prime founders of G&M bank is carnegie(yes/no)= ?",
        "the total workers of G&M is 30000(yes/no)",
        "the worth of G&M is $100000(yes/no = )?"
            
    };
    
    private final String[] ANSWERSLIST = {"yes", "no", 
        "yes", 
        "no", 
        "yes"
    };
    ...
    private class ClientHandler implements Runnable{
        ...
        public void run() {            
            ...
            OUT.println("Kindly wait for at least one or more users......");
                ...
                while(true){
                    ...
                    if(allHaveUsername){
                        if(clients.size()>1 && !firstTimeServerPub){
                            ...
                            Thread.sleep(5000);
                                for(ClientHandler cl : clients){
                                    cl.OUT.println("\n[] now it is time to start  with " + clients.size() + " online users now\n");
                    ...
                    if(incrementQuestion==QUESTIONSLIST.length){
                        ...
                        if(maxMarks>0){
                            if(winners.size()>1){
                                ...
                                Thread.sleep(7000);
                                for(ClientHandler cl: clients){
                                    //cl.OUT.println("[SERVER] says, The Winners For This  are:\n");
                                ...
                                for(ClientHandler cl: clients){
                                    if(cl==singleWinner){
                                        System.out.println("nuber of questions attempted is 6");
                                        System.out.println("the questions correct is 3");
                                        cl.OUT.println("[SERVER] says, The Winner For This  is You and You have got " + maxMarks + "/" + QUESTIONSLIST.length *2 + "\n");
                                    }else{
                                        cl.OUT.println("[SERVER] says, The Winner \n" + singleWinner  +" and has got " + maxMarks + "/" + QUESTIONSLIST.length *2 + "\n");
                                    }
                                }
                            }
                        }else{
